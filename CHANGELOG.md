...
- **Mouse adapter supresses browser default navigation** — the `mouseAdapter`
  now calls `event.preventDefault()` on `mouseup` events when a shortcut is
  matched and a mouse button is pressed, preventing the browser from navigating
  away when a trigger is clicked, or navigating twice if that shortcut
  initiated some for of navigation. 
...
[1.3.1]: https://github.com/fabiankachlock/vue-uni-intent/releases/tag/v1.3.1
