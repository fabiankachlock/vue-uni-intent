...
const keyInput = (k: string, mods: Partial<Record<"ctrl" | "shift" | "alt" | "meta", boolean>> = {}): ShortcutInput => ({
  kind: "key",
  key: k,
...
describe("matchShortcut", () => {
  ...
  it("requires unspecified modifiers to be unpressed", () => {
    ...
    expect(matchShortcut(keyInput("s", { ctrl: true, shift: true }), key("s", { ctrl: true }))).toBe(false);
  });
...
describe("findShortcutTarget", () => {
  const record = (id: string, shortcuts: TriggerRecord["shortcuts"], disabled = false): TriggerRecord => ({
    id,
    layerId: "root",
    element: null,
    isDisabled: () => disabled,
