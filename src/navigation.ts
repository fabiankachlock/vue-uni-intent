...
export function findNext(
  ...
  if (ahead.length > 0) {
    const inCone = ahead.filter(
      (c) =>
        Math.abs(c.cross - origin.cross) <= c.primary - origin.primary || crossAligned(origin, c),
    )
  ...
  if (options.wrap) {
    ...
    // move is a no-op.
    const nearCross = (c: Projected) =>
      crossAligned(origin, c) ||
      Math.abs(c.cross - origin.cross) <=
        (origin.crossEnd - origin.crossStart) * WRAP_CROSS_TOLERANCE
    const behind = projected.filter((c) => c.primary < origin.primary - EPSILON && nearCross(c))
    if (behind.length > 0) {
